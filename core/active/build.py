...
def RegionSelection(cfg, feature_extractor, classifier, tgt_epoch_loader, round_number):
    ...
    # get list of files from dataset
    data_list = tgt_epoch_loader.dataset.data_list
    data_list = random.sample(data_list, 20)
    data_list = [x['name'] for x in data_list]

    VIZ_SET = set(data_list)

    def select_region(tgt_data):
        tgt_input, path2mask = tgt_data['img'], tgt_data['path_to_mask']
