...
class FloatingRegionScore(nn.Module):
    def __init__(
        self, in_channels=19, padding_mode="zeros", size=33, purity_type=None, K=100
    ):
        """
    ...
    def compute_region_uncertainty(self, unc_type, logit, p, ground_truth=None):
        if unc_type == "entropy":
            region_uncertainty = torch.sum(-p * torch.log(p + 1e-6), dim=0).unsqueeze(
                dim=0
            ).unsqueeze(dim=0) / math.log(
                19
            )  # [1, 1, h, w]
            ...
            region_uncertainty = 1 - torch.gather(
                p, dim=0, index=gt_clone.unsqueeze(dim=0)
            ).unsqueeze(dim=0)
        else:
    ...
    def quantize_uncert_map(self, decoder_out):
        ...
        decoder_out_norm = self.mapper.poincare_distance_origin(
            decoder_out, dim=1
        ).squeeze(0)
    ...
    def compute_region_impurity(self, predict, K):
        ...
        region_impurity = torch.sum(
            -dist * torch.log(dist + 1e-6), dim=1, keepdim=True
        ) / math.log(K)
    ...
    def forward(
        ...
        if unc_type == "pixel_entropy":
            region_uncertainty = self.compute_pixel_entropy(p)
        else:
            region_uncertainty = self.compute_region_uncertainty(
                unc_type, logit, p, ground_truth=ground_truth
            )
        ...
        if pur_type == "ripu":
            ...
            region_impurity, count = self.compute_region_impurity(predict, self.K)
        elif pur_type == "none":
            ...
            raise NotImplementedError(
                "Error: purity type '{}' not implemented".format(pur_type)
            )
